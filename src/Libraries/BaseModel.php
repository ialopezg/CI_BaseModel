 ...
 * @version  2.19.0
 * @see      https://github.com/ialopezg/ci_basemodel
 */
class BaseModel extends \CI_Model implements \ArrayAccess {
    /**
    ...
    public function __construct()
    {
        ...
        if ($this->connection) {
            ...
            elseif (is_string($this->connection)) {
                // Cache Mechanism
                if (isset(self::$_dbCaches[$this->connection])) {
                    $this->database = self::$_dbCaches[$this->connection];
                } else {
                    // CI Database Configuration
                    $this->database = get_instance()->load->database($this->connection, true);
                    self::$_dbCaches[$this->connection] = $this->database;
                }
            }
            else {
            ...
            $this->database = $this->_getDefaultDB();
        }
        // Slave
    ...
    protected function fetchTableName() {
        if ((!isset($this->table) || empty($this->table)) && strtolower(get_called_class()) !== 'my_model') {
            $this->table = $this->_dbr->dbprefix($this->getTableName(get_called_class()));
            if (!$this->database->table_exists($this->table)) {
                show_error(
                    sprintf("While trying to figure out the table name, couldn't find an existing table named: <strong>%s</strong>.<br/>You can set the table name in your model by defining the protected variable <strong>" . $this->table . '</strong>.', $this->table),
    ...
    public function insert($attributes, $runValidation=true)
    {
        ...
        return $this->database->insert($this->table, $attributes);
    }
    ...
    public function replace($attributes, $runValidation=true)
    {
        ...
        return $this->database->replace($this->table, $attributes);
    }
    ...
    public function batchUpdate(Array $dataSet, $withAll=false, $maxLength=null, $runValidation=true)
    {
        ...
        foreach ($dataSet as $key => &$each) {
            ...
            if (($count==0 && $sqlBatch) || strlen($sqlBatch)>=$maxLength) {
                // Each batch of query
                $result = $this->database->query($sqlBatch);
                $sqlBatch = "";
    ...
    public function lockForUpdate()
    {
        ...
        return $this->database->query("{$sql} FOR UPDATE");
    }
    ...
    public function sharedLock()
    {
        ...
        return $this->database->query("{$sql} LOCK IN SHARE MODE");
    }
